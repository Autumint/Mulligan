do
    ...
    function G.UIDEF.use_and_sell_buttons(card)
        ...
        if card.config and card.config.center and card.config.center.key == "j_tdec_lunar_locust" then
...
SMODS.Joker {
    ...
    unlocked = true,
    discovered = true,
    config = { extra = { size = 1, mult = 3, light = 0 } },
    loc_vars = function(self, info_queue, card)
        return { vars = { card.ability.extra.size, card.ability.extra.mult, card.ability.extra.mult * math.floor(card.ability.extra.light) } }
    end,
    calculate = function(self, card, context)
        if context.end_of_round then
            card.ability.extra.light = 0
            return {
                message = "Light Upon Ye",
                colour = G.C.BLUE
            }
        end
        ...
        if context.joker_main then
            return {
                mult = card.ability.extra.mult * card.ability.extra.light
            }
        end
    end,
